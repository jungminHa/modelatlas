...
Large downloads do fail partway — Hugging Face's transfer backend returns transient errors. Partial data is kept, so re-running the same command resumes rather than restarts, and `--status` lists any incomplete download with the exact command to resume or discard it.
